...
def Google(query):
    ...
    results = []
    query = query +" after:" + str((datetime.now() - timedelta(days=6)).strftime('%Y-%m-%d'))
...
def ArticleScraper(URL):
    ...
    SelectedTags = Soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p'])
    
    ProcessedTags = []
    for Tag in SelectedTags:
        text = Tag.get_text(strip=True)
        if text:
            ProcessedTags.append(text)
    
    Content = ' '.join(ProcessedTags)
    return Content

def CleanText(Text):
    pattern = re.compile(r'[^a-zA-Z0-9\s\.,!?;:()\'"_-]')
    CleanedString = re.sub(pattern, '', Text)
    Content = ' '.join(CleanedString.split())
    return Content